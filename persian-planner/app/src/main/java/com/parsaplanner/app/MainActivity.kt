...
import com.parsaplanner.app.ui.screens.TimelineScreen
...
sealed class Destination(val route: String, val labelRes: Int) {
    ...
    object Timeline : Destination("timeline", R.string.nav_timeline)
}
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            val themeMode = remember { mutableStateOf(ThemePrefs.getMode(this)) }
            val darkTheme = when (themeMode.value) {
                "dark" -> true
                "light" -> false
                else -> androidx.compose.foundation.isSystemInDarkTheme()
            }
            ParsaPlannerTheme(darkTheme = darkTheme) {
...
@Composable
fun ParsaPlannerApp(onLanguageChanged: (String) -> Unit, onThemeChanged: (String) -> Unit) {
    val navController = rememberNavController()
    val context = LocalContext.current
    val items = listOf(Destination.Tasks, Destination.Notes, Destination.Journal, Destination.Timeline)
    ...
    Scaffold(
                        ...
                        NavigationBarItem(
                            ...
                            icon = {
                                val icon = when (dest) {
                                    ...
                                    Destination.Timeline -> Icons.Filled.Timeline
                                }
        ...
        NavHost(
            ...
            composable(Destination.Timeline.route) { TimelineScreen() }
            composable("settings") {
                SettingsScreen(
